      ...
      <div className="px-5 py-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-mest-grey-700 leading-relaxed">{fr && scenario.executive_summary_fr ? scenario.executive_summary_fr : scenario.executive_summary}</p>
          </div>
        </div>

        {/* Discussion questions */}
        <div className="mt-3 pl-8">
          <p className="text-xs font-semibold text-mest-blue mb-1.5">{t('scenario.discussion')} :</p>
        ...
        >
          ...
          {expanded ? t('scenario.hideBrief') : t('scenario.readBrief')}
        </button>
      </div>
      ...
      {expanded && (
        <div className="border-t border-mest-blue/10 px-5 py-4 space-y-4 bg-white/50">
          ...
          <div className="bg-mest-gold-light rounded-lg p-4">
            <h4 className="text-sm font-semibold text-mest-gold mb-2">{t('scenario.theLesson')}</h4>
            <p className="text-xs mb-2">
              <span className="font-semibold">{t('scenario.trustViolated')} :</span>{' '}
              <span className="bg-mest-gold/20 px-2 py-0.5 rounded">{t(TRUST_LABEL_KEYS[scenario.the_lesson.trust_property_violated] as any)}</span>
            </p>
            <p className="text-xs mb-2">
              <span className="font-semibold">{t('scenario.rebuildTechniques')} :</span>{' '}
              {scenario.the_lesson.rebuild_techniques.map(tech => (
                <span key={tech} className="inline-block bg-white/60 text-mest-grey-700 px-1.5 py-0.5 rounded text-xs mr-1 mb-1">{tech}</span>
              ))}
            </p>
            <p className="text-sm text-mest-grey-700 italic mt-2">{fr && scenario.the_lesson.why_it_matters_for_african_builders_fr ? scenario.the_lesson.why_it_matters_for_african_builders_fr : scenario.the_lesson.why_it_matters_for_african_builders}</p>
          </div>
        </div>
      )}
    </div>
