    ...
    >
      ...
      <div className="px-6 py-4 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold text-sm">Explainability Report</h3>
      ...
      <div className="px-6 py-3 bg-white/5 border-b border-white/5">
        <p className="text-white/50 text-[10px] uppercase tracking-wider">Query</p>
      ...
      <div className="px-6 py-4 space-y-3">
        ...
        >
          <p className="text-white/70 text-xs">
            Question embedded into <span className="text-[#B8860B] font-semibold">{report.layers.query.embeddingDimensions}-dimensional</span> vector for semantic search.
          </p>
        </ReportLayer>
        ...
        >
          ...
          {report.layers.retrieval.topMatch && (
            <div className="bg-white/5 rounded-lg px-3 py-2">
              <p className="text-white/50 text-[10px]">Top match ({report.layers.retrieval.topMatch.similarity}% similarity):</p>
              <p className="text-white/80 text-xs mt-0.5">&ldquo;{report.layers.retrieval.topMatch.text}&rdquo;</p>
              <p className="text-[#B8860B] text-[10px] mt-0.5">from {report.layers.retrieval.topMatch.document}</p>
            </div>
          )}
        </ReportLayer>
        ...
        >
          <div className="grid grid-cols-2 gap-2 text-xs mb-2">
            <div className="bg-white/5 rounded px-2 py-1">
              <span className="text-white/40">Model:</span> <span className="text-white/80">{report.layers.instruction.generationModel}</span>
            </div>
            <div className="bg-white/5 rounded px-2 py-1">
              <span className="text-white/40">Temp:</span> <span className="text-white/80">{report.layers.instruction.temperature}</span>
            </div>
            <div className="bg-white/5 rounded px-2 py-1">
              <span className="text-white/40">Threshold:</span> <span className="text-white/80">{Math.round(report.layers.instruction.strictThreshold * 100)}%</span>
            </div>
            <div className="bg-white/5 rounded px-2 py-1">
              <span className="text-white/40">Top sim:</span>
              <span className={`font-semibold ${report.layers.instruction.thresholdPassed ? 'text-[#0E6B5C]' : 'text-[#922B21]'}`}>
                {' '}{report.layers.instruction.topSimilarity}% {report.layers.instruction.thresholdPassed ? '✓ passed' : '✗ below'}
              </span>
            </div>
          </div>
          <div className="bg-white/5 rounded-lg px-3 py-2">
            <p className="text-white/40 text-[10px]">System prompt:</p>
        ...
        >
          ...
          <p className="text-white/40 text-[10px] mt-1">{report.layers.generation.citationCount} citation{report.layers.generation.citationCount !== 1 ? 's' : ''} in response</p>
        </ReportLayer>
        ...
        >
          {report.layers.evidence.length > 0 ? (
            ...
            <p className="text-white/40 text-xs italic">No citations in response — claims are not traceable.</p>
          )}
        </ReportLayer>
      </div>
      ...
      <div className="px-6 py-4 bg-white/5 border-t border-white/10">
        <h4 className="text-white/50 text-[10px] uppercase tracking-wider mb-2">Confidence Assessment</h4>
        <div className="grid grid-cols-3 gap-3">
          <div className="text-center">
            <p className="text-xs font-bold" style={{ color: confidenceColor }}>{report.confidence.level}</p>
            <p className="text-white/30 text-[9px]">Overall</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-white/70">{report.confidence.citationCoverage}</p>
            <p className="text-white/30 text-[9px]">Citations</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-white/70">{report.confidence.hallucinationRisk}</p>
            <p className="text-white/30 text-[9px]">Hallucination risk</p>
          </div>
        </div>
      </div>
    </motion.div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        {status === 'skipped' && <span className="text-white/30 text-[10px] ml-auto">skipped</span>}
      </div>
      {children}
    </motion.div>
  );
